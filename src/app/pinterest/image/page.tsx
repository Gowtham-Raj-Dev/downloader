...
export default function PinterestImagePage() {
  ...
  const handleFetchProfile = async (inputs: string[]) => {
    ...
    // Identify URLs (We can reuse isPinterestVideoUrl as it matches standard pin links)
    const videoUrls = inputs.filter(input => isPinterestVideoUrl(input));
    ...
    if (!(await checkDailyLimit('single'))) {
    ...
    try {
      const response = await fetch(`/api/pinterest-image?url=${encodeURIComponent(videoUrls[0])}`);
      let result;
      try {
      ...
      if (!response.ok || !result.success || !result.data) {
        ...
        throw new Error(result?.error || `Failed to parse image: ${videoUrls[0]}`);
      }

      const media = result.data.media_details[0];
      const postInfo = result.data.post_info || {};

      const shortcode = extractVideoShortcode(videoUrls[0]) || 'image';
      ...
      const singleItem = {
        ...
        videoUrl: media.url || '', // We map imageUrl to videoUrl property to reuse components
        duration: 'Image',
        views: media.video_view_count || 0,
        likes: postInfo.likes || 0,
        comments: 0,
        caption: postInfo.caption || '',
        type: 'video', // we keep 'video' to satisfy TypeScript, but we treat it as image
      ...
      console.error('Error in single image fetch:', err);
              ...
              >
                ...
                <div className="max-w-4xl mx-auto px-4 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="glass-card p-5 rounded-card border border-neutral-200/40 dark:border-neutral-800/20 text-center">
                    ...
                    <h4 className="text-sm font-bold text-neutral-800 dark:text-neutral-200 mb-1">High Quality Images</h4>
                    <p className="text-xs text-neutral-500 dark:text-neutral-500 leading-relaxed">
                      Paste any pin link to fetch details, view the image in full size, and trigger direct downloads instantly.
                    </p>
                  </div>
                  ...
                  <div className="glass-card p-5 rounded-card border border-neutral-200/40 dark:border-neutral-800/20 text-center">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-500 leading-relaxed">
                      Paste multiple pin links at once to extract and download all photos individually.
                    </p>
                  </div>
                </div>
              </motion.div>
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <h3 className="text-base font-extrabold text-neutral-900 dark:text-white flex items-center gap-2">
                          <ImageIcon className="w-4 h-4 text-[#E60023]" /> Media Found
                        </h3>
