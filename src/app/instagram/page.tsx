...
export default function InstagramPage() {
  ...
  const handleFetchProfile = async (inputs: string[]) => {
    ...
    // Identify video URLs
    const videoUrls = inputs.filter(input => isInstagramVideoUrl(input));
    ...
    try {
      const fetchPromises = videoUrls.map(async (url) => {
        ...
        return {
          ...
          videoUrl: media.url ? `/api/video/stream?url=${encodeURIComponent(media.url)}` : '',
  ...
  const handleDownloadZip = async () => {
    ...
    const confirmZip = window.confirm(`Do you want to download all ${videosToDownload.length} videos as a ZIP file?`);
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute('download', `instagram_videos_${Date.now()}.zip`);
                ...
                >
                  ...
                  <div className="max-w-4xl mx-auto px-4 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="glass-card p-5 rounded-card border border-neutral-200/40 dark:border-neutral-800/20 text-center">
                      ...
                      <p className="text-xs text-neutral-500 dark:text-neutral-500 leading-relaxed">
                        Paste multiple post links at once to extract and download all videos as a batch.
                      </p>
                    </div>
                  </div>
                </motion.div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                          Successfully fetched {fetchedVideos.length} Instagram videos. Preview individually or download all as a single ZIP archive.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-center md:text-left">
            ...
            <p className="text-[10px] text-neutral-400 dark:text-neutral-500 mt-1 max-w-md">
              Disclaimer: This is an educational visual exploration tool. We do not store, host, or crawl copyrighted Instagram media. All assets belong to their respective creators.
            </p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-2 text-xs font-semibold text-neutral-500 dark:text-neutral-500">
            ...
            <span className="text-[10px] text-neutral-400 dark:text-neutral-500 font-mono">
              &copy; {new Date().getFullYear()} InstaExplorer
            </span>
          </div>
        </div>
      </footer>
